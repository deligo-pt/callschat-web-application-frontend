...
export default function ChatRoomPage() {
  ...
  const targetUserId = params.chatId as string;
  
  const [conversationId, setConversationId] = useState<string>("");
  const [recipientId, setRecipientId] = useState<string>(targetUserId);
      ...
      try {
        ...
        if (token) {
           try {
             ...
             if (decoded) {
               setCurrentUserId(decoded.sub || decoded.id || "my-user-id");
             } else {
               setCurrentUserId("my-user-id");
             }
           } catch (e) {
        ...
        // Fetch contacts to get recipient profile
        ...
        const usersArray = contactsData.data?.contacts || contactsData.data || contactsData;
        if (Array.isArray(usersArray)) {
          const match = usersArray.find((u: any) => (u.addressee?.id || u.contact?.id || u.id) === targetUserId);
          if (match) {
            const userProfile = match.addressee?.profile || match.contact?.profile || match.profile || {};
            const displayName = match.customName || userProfile.displayName || userProfile.username || "Unknown";
            
            foundRecipient = {
              id: match.addressee?.id || match.contact?.id || match.id,
              name: displayName,
              avatarUrl: userProfile.avatarUrl || `https://ui-avatars.com/api/?name=U&background=F4F6FC&color=3B58F5`,
        ...
        const convId = res?.data?.id || res?.data?.conversationId || res?.id || res?.conversationId;
        if (convId) {
  ...
  const { messages, sendMessage, isReady } = useChat(conversationId, recipientId);
  ...
  return (
    <div className="flex h-full w-full flex-col bg-[#F8FAFC]">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#E6EAFA]">
              {recipient?.avatarUrl ? (
                <img src={recipient.avatarUrl} alt={recipient.name} className="h-10 w-10 rounded-full object-cover" />
              ) : (
                <div className="text-[#8F95B2] font-bold">{recipient?.name?.charAt(0) || "U"}</div>
              )}
              {recipient?.isOnline && (
            ...
            <div className="flex flex-col">
              <h2 className="text-[15px] font-bold text-[#1D2A54]">{recipient?.name || "Loading..."}</h2>
              <span className="flex items-center gap-1 text-[12px] font-medium text-[#8F95B2]">
                {isInitializing ? "Connecting..." : isReady ? (
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4 bg-[#F0F2F5] relative">
        {/* Chat Background Pattern (Optional WhatsApp-style feel) */}
        ...
        {messages.length === 0 ? (
          ...
          messages.map((msg, index) => {
            ...
            return (
              <div key={msg.id} className={cn("flex w-full z-10", isMe ? "justify-end" : "justify-start")}>
                <div className={cn(
                  ...
                  <span className="leading-snug pr-10 whitespace-pre-wrap word-break" style={{ wordBreak: 'break-word' }}>
                    {msg.text}
                  </span>
                  ...
                  <div className="absolute bottom-1 right-2 flex items-center gap-1">
                    ...
                    {isMe && (
                      <svg viewBox="0 0 16 15" width="16" height="15" className="text-[#53bdeb]">
                        <path fill="currentColor" d="M15.01 3.316l-.478-.372a.365.365 0 0 0-.51.063L8.666 9.879a.32.32 0 0 1-.484.033l-.358-.325a.319.319 0 0 0-.484.032l-.378.483a.418.418 0 0 0 .036.541l1.32 1.199c.143.13.36.125.498-.011l5.3-6.974a.418.418 0 0 0-.106-.54z"></path>
                        <path fill="currentColor" d="M11.51 3.316l-.478-.372a.365.365 0 0 0-.51.063L5.166 9.879a.32.32 0 0 1-.484.033L2.01 7.489a.319.319 0 0 0-.484.032l-.378.483a.418.418 0 0 0 .036.541l3.052 2.776c.143.13.36.125.498-.011l5.3-6.974a.418.418 0 0 0-.106-.54z"></path>
                      </svg>
                    )}
                  </div>
                </div>
