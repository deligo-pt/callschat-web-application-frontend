...
import { Send, Paperclip, Camera, Mic, Square, X, Loader2, Image as ImageIcon } from "lucide-react";
  ...
  return (
    <div className="bg-[#F0F2F5] px-4 py-3 shrink-0 relative">
      ...
      <form onSubmit={handleSend} className="flex items-end gap-2">
        ...
        {!isRecording && (
          <div className="flex gap-1 mb-1">
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-full text-[#8F95B2] hover:bg-[#E6EAFA] transition-colors disabled:opacity-50"
            >
        ...
        <div className="flex-1 bg-white rounded-2xl flex items-center px-4 py-2 shadow-sm min-h-[44px]">
          {isRecording ? (
        ...
        {inputText.trim() === "" && !selectedFile && !isUploading ? (
          <button
            ...
            disabled={!isReady}
            className={cn(
              "flex h-11 w-11 shrink-0 items-center justify-center rounded-full transition-transform hover:scale-105 disabled:opacity-50 disabled:hover:scale-100 shadow-sm mb-0.5",
              isRecording ? "bg-red-500 text-white" : "bg-[#E6EAFA] text-[#3B58F5]"
            )}
          >
            {isRecording ? (
          ...
          <button
            ...
            className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#00A884] text-white transition-transform hover:scale-105 disabled:opacity-50 disabled:hover:scale-100 shadow-sm mb-0.5"
          >
            {isUploading ? (
