...
export default function PersonalLoginScreen() {
        ...
        if (data.success && token) {
          if (isExistingUser || existingAccountType) {
            if (existingAccountType === "BUSINESS") {
              toast.error(
                "This phone number is registered as a Business account. Please login in Business Mode."
              );
              return;
            }
            ...
            router.push("/auth/personal/signup");
          }
        } else {
          toast.error(
  ...
  const handleOtpChange = (idx: number, val: string) => {
    ...
    setOtp(next);
    if (val && idx < 5)
      document.getElementById(`pl-otp-${idx + 1}`)?.focus();
  };
  ...
  const handleOtpPaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    setOtp(next);
    document.getElementById(`pl-otp-${Math.min(pasted.length, 5)}`)?.focus();
  };
        ...
        {step === "PHONE" && (
          <>
            ...
            <p className="text-blue-100 font-medium text-center text-sm sm:text-base">
              Welcome back to your private chats
            </p>
          </>
        )}
          ...
          <div className="w-full max-w-[420px] flex-1 flex flex-col pt-2 sm:pt-4">
            <div className="space-y-1.5 mb-6">
              ...
              <p className="text-sm font-medium text-[#64748B] leading-relaxed">
                Enter your registered phone number to sign in to your encrypted conversations.
              </p>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative" ref={dropdownRef}>
                  <button
                    ...
                    className="flex h-13 w-[105px] items-center justify-between rounded-xl border border-indigo-100 bg-[#EEF2FF] px-3 transition-all focus:border-[#2563EB] focus:bg-white focus:ring-2 focus:ring-blue-500/20 hover:border-indigo-200 cursor-pointer"
                  >
                ...
                <Input
                  ...
                  disabled={isPending}
                  className="flex h-13 flex-1 rounded-xl border border-indigo-100 bg-[#EEF2FF] px-4 text-sm font-semibold text-slate-800 placeholder-slate-400 transition-all focus:border-[#2563EB] focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 hover:border-indigo-200"
                />
              </div>
            </div>
            ...
            <div className="flex justify-center gap-2 sm:gap-3 mb-8">
              {otp.map((digit, idx) => (
                <input
                  ...
                  className="h-13 w-11 sm:h-14 sm:w-13 rounded-xl border border-indigo-100 bg-[#EEF2FF] text-center text-xl font-bold text-[#1E293B] transition-all focus:border-[#2563EB] focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                />
              ))}
            </div>
